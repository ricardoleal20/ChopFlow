...
# the crates.io source tarball. The `chopflow` crate's path dependencies are
# rewritten to registry dependencies at `cargo publish` time, so the source
# build resolves everything from crates.io. protoc is vendored via
# `protoc-bin-vendored`, so no system protobuf is needed.
...
class Chopflow < Formula
  ...
  # The crate is binary-only (one [[bin]] target: `chopflow`), so it carries a
